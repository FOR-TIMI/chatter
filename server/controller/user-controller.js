...
const userController = {
    ...
    async getAllUsers(req,res){
        try{
            ...
            res.status(500).json({ message: "something went wrong with the server", error})
        }
    },
    ...
    async createNewUser({body}, res){
        ...
        if( username && email && password){
            ...
            res.status(422).json({message: "Invalid fields", "possible Solution": "make sure that the username,email and password fields are not NULL"})
        }
        ...
        try{
           //To update the person i'm unfollowing's follower list
            ...
            if(!unfollowedUser){
                return res
                    .status(404)
                    .json({message: "Could not find the user you're trying to unfollow" })
            }
